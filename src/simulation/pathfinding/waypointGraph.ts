...
export class WaypointNavigator {
  ...
  // 가장 최근 selectNextNode 호출에서 force-exit redirect 가 발화했다면 그 사유.
  ...
  selectNextNode(
    ...
    if (stuck || budgetExceeded || allEssentialDone) {
      ...
      if (exitFirstHop) {
        // Disambiguate the dominant cause for downstream diagnostics.
        // stuck 은 STUCK_AT_NODE_MS OR MAX_TOTAL_DWELL_MS 둘을 포괄하므로
        // totalDwell 기준으로 maxDwell vs nodeStuck 분리.
        const totalDwell = now > 0 ? now - visitor.enteredAt : 0;
        if (totalDwell >= MAX_TOTAL_DWELL_MS) this._lastForceExit = 'maxDwell';
        else if (stuck) this._lastForceExit = 'nodeStuck';
        else if (budgetExceeded) this._lastForceExit = 'budgetExceeded';
        else this._lastForceExit = 'allEssentialDone';
        return exitFirstHop;
      }
    }
  ...
  /**
   * 가장 최근 selectNextNode 호출에서 force-exit redirect 가 발화했으면 그 사유.
