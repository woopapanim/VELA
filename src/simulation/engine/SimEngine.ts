...
export class SimulationEngine {
  ...
  private pushOutsideMedia(pos: Vector2D, m: MediaPlacement): Vector2D {
    ...
    if (this.isMediaEllipse(m)) {
      // Push radially along ellipse from center; scale to boundary then +1px
      const a = m.size.width * MEDIA_SCALE / 2;
      const b = m.size.height * MEDIA_SCALE / 2;
      const rad = (m.orientation * Math.PI) / 180;
      const cos = Math.cos(rad), sin = Math.sin(rad);
      ...
      // scale point to ellipse surface, then push +1px outward along same direction
      const boundaryLen = Math.sqrt(lx * lx + ly * ly) / k;
      const scale = (boundaryLen + 1) / (Math.sqrt(lx * lx + ly * ly) || 1);
