...
import { LayoutGrid, Play, BarChart3 } from 'lucide-react';
...
interface Props {
  current: Stage;
  nav: StageNav;
  /** disabled 상태 stage 에 hover 시 표시할 hint. */
  navDisabledHints?: Partial<Record<Stage, string | undefined>>;
  /** disabled stage 클릭 시 호출 — hint 를 toast 등으로 가시화. */
  onNavDisabledClick?: (stage: Stage, hint: string) => void;
  /** 하단에 붙는 stage-internal 도구. Build 의 task icons 등. */
  bottom?: ReactNode;
}

const STAGES: { id: Stage; icon: typeof LayoutGrid; label: string }[] = [
  { id: 'build', icon: LayoutGrid, label: '1 Build' },
...
export function StageRail({ current, nav, navDisabledHints, onNavDisabledClick, bottom }: Props) {
  return (
    <aside className="w-14 border-r border-border bg-[var(--surface)] flex flex-col flex-shrink-0">
      <div className="flex flex-col items-center gap-1 py-2">
        {STAGES.map(({ id, icon: Icon, label }) => {
          ...
          const disabledHint = !isCurrent && !clickable ? navDisabledHints?.[id] : undefined;
          const onDisabled = disabledHint && onNavDisabledClick
            ? () => onNavDisabledClick(id, disabledHint)
            : undefined;
          return (
            <button
              key={id}
              type="button"
              onClick={handler ?? onDisabled}
              disabled={!clickable && !onDisabled}
              ...
              className={`relative w-10 h-10 rounded-lg flex items-center justify-center transition-colors
                ${
                  isCurrent
                    ...
                    : disabledHint
                    ? 'text-muted-foreground/40 hover:text-muted-foreground hover:bg-secondary/40 cursor-help'
