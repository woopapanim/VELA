...
export function CompletionReport() {
  ...
  const guardCompleted = useCallback((fn: () => void) => {
    if (!isCompleted) {
      alert(t('completionReport.analyzing'));
      return;
    }
    fn();
  }, [isCompleted, t]);
    ...
    return (
      <div className="bento-box p-3">
        <h2 className="panel-section flex items-center gap-1.5 mb-2">
          <FileText className="w-3.5 h-3.5" /> {t('completionReport.title')}
        </h2>
        <button
          onClick={() => guardCompleted(() => setShowFullReport(true))}
          disabled={!scenario}
          className="w-full flex items-center justify-center gap-1.5 px-3 py-2 mb-2 text-[11px] font-medium rounded-lg bg-primary/15 hover:bg-primary/25 text-primary ring-1 ring-primary/30 disabled:opacity-40 transition-colors"
        >
          <Maximize2 className="w-3 h-3" />
          {t('completionReport.viewFull')}
        </button>
  ...
  return (
    <div className="bento-box p-3">
      ...
      {/* Full Report launcher */}
      <button
        onClick={() => guardCompleted(() => setShowFullReport(true))}
        className={`w-full flex items-center justify-center gap-1.5 px-3 py-2 mb-3 text-[11px] font-medium rounded-lg ring-1 transition-colors ${
          isCompleted
            ? 'bg-primary/15 hover:bg-primary/25 text-primary ring-primary/30'
            : 'bg-secondary text-muted-foreground ring-border cursor-not-allowed opacity-60'
        }`}
      >
        <Maximize2 className="w-3 h-3" />
        {t('completionReport.viewFull')}
      ...
      <div className="grid grid-cols-4 gap-1 mb-3">
        <ExportBtn label="JSON" icon={FileDown} onClick={() => guardCompleted(handleExportJson)} primary dimmed={!isCompleted} />
        <ExportBtn label="Zone" icon={TableProperties} onClick={() => guardCompleted(handleExportZone)} dimmed={!isCompleted || !hasHistory} />
        <ExportBtn label="Visitor" icon={TableProperties} onClick={() => guardCompleted(handleExportVisitor)} dimmed={!isCompleted} />
        <ExportBtn label="Media" icon={TableProperties} onClick={() => guardCompleted(handleExportMedia)} dimmed={!isCompleted} />
      </div>
...
function ExportBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center gap-0.5 px-1 py-1.5 text-[9px] font-medium rounded-lg transition-colors ${
        dimmed
